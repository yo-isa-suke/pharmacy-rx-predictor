"""
薬局 年間処方箋枚数 多面的予測ツール v2.2
==========================================
v2.1 からの改善点:
  1. マップ精度改善
     - 国土地理院（GSI）ジオコーダーを第一優先に採用（日本住所向け高精度API）
     - Nominatim を第二候補（フォールバック）に変更
     - マップポップアップに Google Maps リンクを追加
  2. 統計数値の根拠・データソース表示
     - 処方箋発行率(65%)・院外処方率(74.5%)等の出典を詳細表示
  3. 新規開局予測モード（スーパー内調剤薬局など）
     - 開局予定住所を入力するだけで処方箋枚数を予測
     - シナリオA: 門前クリニック誘致モード
       （誘致する診療科・外来患者数を設定 → 方法①ロジックで予測）
     - シナリオB: 面での集客モード（商圏人口動態 → 方法②ロジックで予測）
     - シナリオC: 両方同時実行（比較）
     - 近隣薬局の処方箋枚数をMHLWから取得してマップに表示
"""
...
STAT_REFERENCES: Dict[str, Dict] = {
    ...
    "年齢層別外来受診率": {
        ...
        "note": "推計外来患者数を人口で除した受診率。75歳以上は慢性疾患による継続受診が多く、最も高い受診率を示す。次回調査は2023年。",
    },
    "日本の年齢分布": {
...
class Method2Predictor:
    ...
    def _market_share(self, lat, lon, competitors) -> Tuple[float, str]:
        if not competitors:
            return 0.65, "商圏内競合なし（独占補正0.65）"
        cws = [1.0 / max(p.distance_m, 20) ** 2 for p in competitors]
        share = min(1.0 / (1.0 + sum(cws)), 0.80)
        return share, f"距離重み付きシェア（競合{len(competitors)}件）: {share:.1%}"
...
def main() -> None:
    st.set_page_config(
        page_title="薬局 処方箋枚数 多面的予測 v2.2",
        page_icon="🔬",
        layout="wide",
        initial_sidebar_state="collapsed",
    )
    st.title("🔬 薬局 年間処方箋枚数 多面的予測ツール v2.2")
    st.markdown(
        "🆕 **v2.2の改善点**: 国土地理院ジオコーダーでマップ精度向上 / "
        "統計数値のデータソース表示 / **新規開局予測モード（スーパー内薬局等）追加**"
    )
