"""
薬局 年間処方箋枚数 多面的予測ツール v3.1
==========================================
v2.6 からの改善点（パラメータ精度向上・根拠強化）:
  1. 院外処方率を最新値に更新: 74.5% → 79.0%
     出典: 厚生労働省「社会医療診療行為別統計」令和4（2022）年
     （方法①・方法② 両方に影響）
  2. 処方箋発行率を診療科加重平均に基づき修正: 65% → 69%
     診療科別処方率（SPECIALTY_RX_RATES）の診療科シェア加重平均
     （歯科を除いた医科受診分のみで算出）
     （方法② に影響）
  3. 方法②市場シェア上限を実態に合わせて修正: 55% → 80%
     競合薬局ゼロの門前薬局では80%前後のシェアが実態に即している。
     競合が1社以上ある場合は距離帯別モデルが適用されるため影響なし。
  4. MHLW補填施設の外来患者数デフォルト値を修正: 50人/日 → 40人/日
     一般的な個人クリニックの外来規模（30〜60人/日）の中央値に近い40人を採用。
  5. 予測結果に推計レンジの注釈コメントを追加（⑤ 透明性確保）
...
class OverpassSearcher:
    ...
    def search_nearby(
        ...
        # v2.4: 日本語タグ・追加 healthcare タグを拡充
        query = f"""
[out:json][timeout:35];
(
...
class Method2Predictor:
    def predict(
        ...
        methodology = [
            "### 方法②（商圏人口動態アプローチ）ロジック",
            "",
            "**算出式**: 商圏人口 × 年齢層別受診率 × 処方箋発行率(69%・医科加重平均)",
            "× 院外処方率(79.0%) × 当薬局市場シェア",
            "",
    ...
    def _market_share(self, lat, lon, competitors) -> Tuple[float, str]:
        ...
        v2.3 修正: 距離帯別に競合を「実効競合数」に換算して Huff型シェアを計算する。
            ...
            シェア = 1 / (実効競合数 + 1)
            上限: 80% （競合ゼロ門前。残20%は商圏外流出・他エリア受診として控除: v3.1修正）
            下限:  8% （極めて競合が多い場合でも最低限を確保）
        """
        if not competitors:
            return 0.80, "商圏内競合なし（上限80%: 門前独占。残20%は商圏外流出・他エリア受診を考慮）"
        ...
        # 上限80% / 下限8% でクリップ（v3.1: 競合ゼロ時も同上限を適用）
...
def fetch_mhlw_medical_supplement(
    ...
    for cand, lat, lon, dist in geocoded:
        ...
        is_hospital = any(kw in cand.name for kw in ["病院", "医療センター", "医療機構", "クリニック" in cand.name and False])
...
def render_comparison_banner(analysis: FullAnalysis) -> None:
    ...
    with cols[1]:
        if m1:
            ...
            st.caption("📌 精度±30〜40%。近隣施設数・患者数の精度に依存")
    with cols[2]:
        if m2:
            ...
            st.caption("📌 精度±40〜50%。商圏人口・競合状況の推計誤差を含む")
    with cols[3]:
...
def render_gap_and_manual_input(analysis: "FullAnalysis") -> None:
    ...
    if implied:
        st.warning(
            ...
            f"近隣に OSM で未収録の医療施設がある可能性があります。\n"
            f"乖離から逆算すると、**外来 約{implied['implied_outpatients']}人/日** の"
            f"クリニック（一般内科想定）が検出されていない可能性があります。\n\n"
            f"👇 **「MHLWエリア自動補填」ボタン**で厚労省DBから一括取得・補正できます。"
        )

    # ─── v2.6: MHLWエリア自動補填セクション ────────────────────────────────
    mhlw_facs: List[NearbyFacility] = st.session_state.get("mhlw_supplement", [])

    with st.expander(
        f"🔄 MHLWデータベースから医療機関を自動補填"
        + (f"（✅ {len(mhlw_facs)}件補填済み）" if mhlw_facs else ""),
        expanded=bool(implied and not mhlw_facs),
    ):
        st.markdown(
            "厚労省「医療機能情報提供制度」データベースから**地域の医療機関を一括取得**し、"
            "OSMで未収録の施設を自動的に補填します。\n\n"
            "| 処理ステップ | 内容 |\n"
            "|---|---|\n"
            "| ① MHLW検索 | 住所の市区町村名でMHLWを検索（医療機関全件） |\n"
            "| ② ジオコーディング | 各施設の住所をGSIで座標変換 |\n"
            "| ③ 半径フィルタ | 薬局から商圏半径+200m以内の施設のみ保持 |\n"
            "| ④ OSM重複排除 | 80m以内に既存OSM施設がある場合はスキップ |\n"
            "| ⑤ 自動追加 | 残りをMHLW補填施設として方法①に反映 |\n\n"
            "⏱ 目安時間: 候補数によって **30秒〜2分** 程度かかります。"
        )

        if mhlw_facs:
            st.success(f"✅ {len(mhlw_facs)}件のMHLW医療機関を補填済みです")
        ...
        if not analysis.pharmacy_lat:
            st.warning("座標が未取得のため自動補填を実行できません。")
        else:
            search_r = analysis.commercial_radius + 200
            st.caption(
                f"検索住所: {analysis.pharmacy_address[:40]}… | "
                f"エリアキーワード: **{extract_area_keyword(analysis.pharmacy_address)}** | "
                f"補填半径: {search_r}m"
            )
            if st.button(
                "🔍 MHLWエリア自動補填を実行", key="mhlw_auto_supplement_btn",
                type="primary", use_container_width=True,
            ):
                progress = st.progress(0, text="MHLW医療機関データベースを検索中…")
                ...
                st.session_state.setdefault("mhlw_supplement_log", []).extend(sup_log)
                progress.empty()
                ...
                if new_facs:
                    ...
                    st.info(
                        "新規の医療機関は見つかりませんでした。\n"
                        "OSMのデータが充分な地域か、または住所のエリアキーワードを"
                        "変えて手動検索をお試しください。"
                    )
                    for msg in sup_log[-5:]:
                        st.caption(msg)
...
def main() -> None:
    st.set_page_config(
        page_title="薬局 処方箋枚数 多面的予測 v3.1",
        page_icon="🔬",
        layout="wide",
        initial_sidebar_state="collapsed",
    )
    st.title("🔬 薬局 年間処方箋枚数 多面的予測ツール v3.1")
    st.markdown(
        "📊 **v3.1 パラメータ精度向上**: "
        "院外処方率を最新値（**79.0%** 令和4年度）に更新、"
        "処方箋発行率を診療科加重平均（**69%**）に更新、"
        "方法②シェア上限を**80%**（競合ゼロ門前実態）に修正。"
    )

    for k, v in [
        ("candidates", []), ("analysis", None), ("new_result", None),
        ("manual_facility_params", []),   # v2.4: 手動追加施設パラメータ
        ("clinic_candidates_v4", []),     # v2.4: MHLWクリニック検索結果
        ("mhlw_supplement", []),          # v2.6: MHLW自動補填施設リスト
        ("mhlw_supplement_log", []),      # v2.6: 補填ログ
    ]:
...
def _render_new_pharmacy_mode() -> None:
    ...
    fetch_mhlw_supp = st.checkbox(
        "🔄 MHLWデータベースから近隣医療機関を自動補填（約30秒〜2分、精度が上がります）",
        value=False,
        key="new_fetch_mhlw_supp",
        help=(
            "ONにすると厚労省「医療機能情報提供制度」DBからエリア内の医療機関を一括取得し、"
            "OSMで未収録の施設を自動補填します。方法①の推計精度が向上します。"
        ),
    )
    ...
    if st.button(
        ...
        config = NewPharmacyConfig(
            ...
            fetch_mhlw_supplement=fetch_mhlw_supp,
        )
...
def run_analysis(candidate: PharmacyCandidate, try_mhlw_medical: bool) -> None:
    ...
    # F: 予測（v2.4: 手動追加施設を含めて計算）
    progress.progress(78, text="[6/6] 方法①②を計算中…")

    # 事前に手動追加済みの施設があれば初期計算にも含める
    ...
    merged_medical = nearby_medical + manual_facs

    m1 = Method1Predictor().predict(
        lat or 0.0, lon or 0.0, merged_medical, nearby_pharmacies,
        mode_label="方法①: 近隣医療機関アプローチ" + ("（手動追加含む）" if manual_facs else ""),
    ) if lat else None
    m2 = Method2Predictor().predict(
...
def run_new_pharmacy_analysis(config: NewPharmacyConfig) -> None:
    ...
    if lat and lon:
        ...
        # v2.6: MHLW医療機関エリア自動補填（オプション）
        if config.fetch_mhlw_supplement and lat:
            progress.progress(58, text="[3.7/5] MHLWから近隣医療機関を自動補填中…")
            pref_code = ""
            for pref_name, pc in PREFECTURE_CODES.items():
