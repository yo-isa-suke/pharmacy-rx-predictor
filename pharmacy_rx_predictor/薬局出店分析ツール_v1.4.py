...
2つの独立した方法で見積もって比較する。
  ...
  ② 集客ベース（来店客数）：店舗の来店客のうち何割が処方箋を持ち込むかで見積もる。
     年齢層別（会員データがあれば任意の粒度）に受診回数を積み上げる。
...
v1.4 変更点 (2026-08-26):
- 【年齢区分】②集客ベースの年齢区分を「65歳以上／未満」の2区分固定から、**任意区分**に一般化。
  スーパー様から年齢層別の会員数をいただけたときに、その粒度のまま反映できる。
  ・サイドバーで「かんたん（65歳以上／未満）」／「詳細（年齢層別）」を切替。
  ・詳細モードは表形式（年齢層・下限年齢・実人数・構成比・月受診回数）で行を自由に追加/削除。
  ・**実人数（会員数）を入れれば構成比は自動計算**。実人数が1行も無ければ構成比の列を使用。
  ・構成比は常に合計1.0へ正規化するので、％でも人数でも端数を気にせず入力できる。
  ・客数の絶対量は従来どおり「月間ユニーク客数 × 周知率」。年齢表が変えるのは**内訳だけ**。
- 【互換性】既定は従来どおり2区分。詳細モードの既定値は
  「形状＝外来受療率の年齢プロファイル／水準＝現行の2区分値(65+ 3.0回/月・65− 1.3回/月)」に
  較正してあるため、全国の年齢構成では2区分版と年間受診回数が一致する（差0.01%未満）。
  → モードを切り替えただけで過去の見積りが動くことはない。
- 【候補地別】候補地ごとに年齢構成を差し替えられるようにした（店舗別の会員データ用）。
  候補地の詳細 →「👥 この候補地だけ年齢構成を差し替える」。共通設定に戻すボタンつき。
- 【内訳表示】②の内訳に「年齢層別の内訳」表を追加。どの年齢層が枚数を生んでいるかが分かる
  （有効客数・年間受診延べ・院外処方プール・獲得枚数・枚数への寄与）。
- 【Excel】「集客_」シートを年齢区分の数に合わせて伸縮する数式レイアウトに変更。
  年齢層の構成比・月受診回数を編集すると獲得枚数まで自動再計算される。
...
st.set_page_config(page_title="薬局 出店候補地 分析ツール v1.4", page_icon="🏪", layout="wide")
...
# ─── v1.4: 年齢層別モデル（65歳以上/未満の2区分固定をやめ、任意区分に一般化） ──────
# スーパー様から「年齢層別の会員数」をいただけるケースがあるため、区分数を固定しない。
#
# 既定の詳細区分は
#   ・構成比 … 全国の年齢構成（65歳以上の合計を2区分既定と同じ0.300に揃えてある）
#   ・月受診回数の形状 … 外来受療率の年齢プロファイル（相対比）
#   ・月受診回数の水準 … 現行2区分の既定値（65歳以上 3.0回/月・65歳未満 1.3回/月）に一致するよう較正
# としてある。既定のまま「かんたん」→「詳細」に切り替えても年間受診回数の差は0.05%未満なので、
# モードを変えただけで過去の見積りが動くことはない。実データがあれば必ず上書きして使うこと。

SENIOR_AGE_FROM = 65        # 「65歳以上」として集計する下限年齢（表示・後方互換用）


@dataclass
class AgeBand:
    """年齢区分1つ分。ratio は構成比（使用時に合計1.0へ正規化される）。"""
    label: str
    ratio: float
    visits_month: float
    age_from: int = 0                  # 区分の下限年齢（65+集計・並べ替えに使用）
    members: Optional[float] = None    # 会員数などの実人数（入力の記録用。按分は ratio 側で行う）

    @property
    def is_senior(self) -> bool:
        return self.age_from >= SENIOR_AGE_FROM


# 既定①：かんたん（2区分）＝ v1.3 までと完全に同一の挙動
DEFAULT_R65, DEFAULT_V65, DEFAULT_VU65 = 0.30, 3.0, 1.3

# 既定②：詳細（年齢層別）。会員データをもらえたときの出発点。
#   ラベル, 下限年齢, 構成比, 月受診回数
# 構成比は合計ちょうど1.000・65歳以上の合計ちょうど0.300（＝2区分既定の高齢化率）に揃えてある。
DEFAULT_AGE_BANDS_DETAIL: List[Tuple[str, int, float, float]] = [
    ("〜19歳",    0, 0.1668, 1.34),
    ("20〜29歳", 20, 0.0948, 0.77),
    ("30〜39歳", 30, 0.1096, 0.96),
    ("40〜49歳", 40, 0.1422, 1.15),
    ("50〜59歳", 50, 0.1283, 1.66),
    ("60〜64歳", 60, 0.0583, 2.24),
    ("65〜74歳", 65, 0.1448, 2.52),
    ("75歳以上", 75, 0.1552, 3.45),
]


def two_band_ages(ratio_65plus: float = DEFAULT_R65,
                  visits_65plus: float = DEFAULT_V65,
                  visits_under65: float = DEFAULT_VU65) -> List["AgeBand"]:
    """v1.3までの2区分をそのまま AgeBand 化する（式・結果とも従来と完全一致）。"""
    r = min(max(float(ratio_65plus), 0.0), 1.0)
    return [
        AgeBand("65歳未満", 1.0 - r, float(visits_under65), age_from=0),
        AgeBand("65歳以上", r, float(visits_65plus), age_from=SENIOR_AGE_FROM),
    ]


def detail_bands_default() -> List["AgeBand"]:
    """既定の詳細（年齢層別）区分。"""
    return [AgeBand(lab, ratio, v, age_from=af)
            for lab, af, ratio, v in DEFAULT_AGE_BANDS_DETAIL]


def normalize_age_bands(bands: Optional[List["AgeBand"]]) -> List["AgeBand"]:
    """構成比の合計を1.0に正規化する。空・合計0なら従来の2区分既定にフォールバック。"""
    valid = [b for b in (bands or []) if b.ratio is not None and b.ratio > 0]
    total = sum(b.ratio for b in valid)
    if not valid or total <= 0:
        return two_band_ages()
    return [AgeBand(b.label, b.ratio / total, max(float(b.visits_month or 0.0), 0.0),
                    b.age_from, b.members) for b in valid]


@dataclass
class FootfallParams:
    """集客ベース（来店客数）予測のパラメータ。
    v1.4：年齢2区分固定をやめ、任意区分の age_bands（構成比×月受診回数）に一般化した。"""
    enabled: bool = False
    store_format: str = "食品スーパー"
    unique_customers_monthly: float = 0.0   # 月間ユニーク客数（会員数 or POS客数÷来店回数で算出）
    age_bands: List[AgeBand] = field(default_factory=two_band_ages)  # v1.4: 既定=従来の2区分
    ...
    # ── 後方互換：v1.3までのフィールド名は age_bands からの導出値として読めるようにする ──
    @property
    def bands(self) -> List[AgeBand]:
        return normalize_age_bands(self.age_bands)

    @property
    def ratio_65plus(self) -> float:
        return sum(b.ratio for b in self.bands if b.is_senior)

    @property
    def visits_month_65plus(self) -> float:
        bs = [b for b in self.bands if b.is_senior]
        w = sum(b.ratio for b in bs)
        return (sum(b.ratio * b.visits_month for b in bs) / w) if w > 0 else 0.0

    @property
    def visits_month_under65(self) -> float:
        bs = [b for b in self.bands if not b.is_senior]
        w = sum(b.ratio for b in bs)
        return (sum(b.ratio * b.visits_month for b in bs) / w) if w > 0 else 0.0

    @property
    def is_detailed_ages(self) -> bool:
        """3区分以上＝年齢層別モードで動いている、の判定（表示の出し分けに使用）。"""
...
def compute_footfall_prediction(
    ...
    v1.4：年齢区分の数を固定しない一般形にした（2区分でも8区分でも同じ式）。
        獲得 = [Σ_年齢区分(ユニーク客数 × 構成比_i × 月受診回数_i × 12)] × 発行率 × 院外率
               × 当該薬局利用率 ÷ (競合面薬局パワー + 1)
    ※ 2区分（65歳以上/未満）を入れれば v1.3 と完全に同じ数値になる。
    """
    if not fp.enabled or fp.unique_customers_monthly <= 0:
        return None
    u = fp.unique_customers_monthly
    bands = normalize_age_bands(fp.age_bands)
    rows, annual_visits = [], 0.0
    for b in bands:
        cust = u * b.ratio
        av = cust * b.visits_month * 12.0
        annual_visits += av
        rows.append({"label": b.label, "ratio": b.ratio, "visits_month": b.visits_month,
                     "customers": cust, "annual_visits": av,
                     "age_from": b.age_from, "is_senior": b.is_senior,
                     "members": b.members})
    ...
    u65 = sum(r["customers"] for r in rows if r["is_senior"])
    return {
        "total": total, "annual_visits": annual_visits, "rx_pool": rx_pool,
        "denom": denom, "u65": u65, "u_under": u - u65, "share": fp.use_rate / denom,
        "bands": rows, "n_bands": len(rows),
    }
...
class MHLWScraper:
    ...
    def search_pharmacies_by_latlon(
        ...
        def _page(p: int) -> Optional[str]:
            try:
                r = self._sess().get(
                    ...
                    params={"id": search_id, "page": p, "size": 20, "sortNo": 2},
                    timeout=15,
                )
        ...
        first = _page(0)
        if first:
            phs, total = self._parse_pharmacy_list(first)
            all_ph.extend(phs)
        n_pages = min(max_pages, math.ceil(total / 20) if total else 1)
        if n_pages > 1 and all_ph:
            with ThreadPoolExecutor(max_workers=min(6, n_pages - 1)) as ex:
                for p, html in zip(range(1, n_pages),
                                   ex.map(_page, range(1, n_pages))):
                    if html:
                        phs, _ = self._parse_pharmacy_list(html)
                        all_ph.extend(phs)
    ...
    def search_medical_by_latlon(
        ...
        def _page(p: int) -> Optional[str]:
            try:
                r = self._sess().get(
                    f"{redirect_url}{sep}page={p}&size=20&sortNo=2", timeout=15)
                return r.text if r.status_code == 200 else None
            except Exception:
                return None
        ...
        first = _page(0)
        if first:
            facs, total = self._parse_med_list(first)
            all_facs.extend(facs)
        n_pages = min(max_pages, math.ceil(total / 20) if total else 1)
        if n_pages > 1 and all_facs:
            with ThreadPoolExecutor(max_workers=min(6, n_pages - 1)) as ex:
                for html in ex.map(_page, range(1, n_pages)):
                    if html:
                        facs, _ = self._parse_med_list(html)
                        all_facs.extend(facs)
        dist_str = f"{radius_m // 1000}km" if radius_m >= 1000 else f"{radius_m}m"
...
def run_analysis(
    ...
    if use_osm:
        ...
        if not med_osm:
            log.append("[Step2-3] ⚠️ OSM医療機関0件（応答なし/圏内になし）→ ナビィデータのみで処理します")
    else:
    ...
    _parallel(med_targets, _fetch_med, workers, _med_prog)
    ...
    # ─────────────────────────────────────────────────────────────────────
    if not verify_pass:
        log.append("[Step6-7] 推考（念のため再検索）はスキップしました"
                   "（同条件の再検索のため結果は同じ・サイドバーでONにできます）。")
    else:
        ...
        for vf in add_meds:
            ...
            med_osm.append(vf)
        log.append(
            f"[Step6] 推考①: 医療機関 {len(add_meds)}件追加 "
...
# ════════════ v1.4: 年齢層テーブル（DataFrame） ⇄ AgeBand の変換 ════════════
AGE_DF_COLS = ["年齢層", "下限年齢", "実人数", "構成比(%)", "月受診回数"]


def age_bands_to_df(bands):
    """AgeBand のリストを編集用 DataFrame にする。"""
    return pd.DataFrame([{
        "年齢層": b.label,
        "下限年齢": int(b.age_from),
        "実人数": (float(b.members) if b.members else None),
        "構成比(%)": round(b.ratio * 100.0, 2),
        "月受診回数": round(float(b.visits_month), 2),
    } for b in bands], columns=AGE_DF_COLS)


def bands_from_df(df):
    """編集用 DataFrame から AgeBand を作る。
    「実人数」が1行でも入っていればそれを構成比の基準にし（実人数が空の行は0扱い）、
    1行も無ければ「構成比(%)」列を使う。いずれも最後に合計1.0へ正規化する。"""
    recs = []
    for _, row in df.iterrows():
        lab = str(row.get("年齢層") or "").strip()
        if not lab:
            continue
        af, pct = _num(row.get("下限年齢")), _num(row.get("構成比(%)"))
        mem, vis = _num(row.get("実人数")), _num(row.get("月受診回数"))
        recs.append({"label": lab, "age_from": int(af) if af is not None else 0,
                     "pct": float(pct) if pct is not None else 0.0,
                     "members": (float(mem) if (mem is not None and mem > 0) else None),
                     "visits": float(vis) if vis is not None else 0.0})
    if not recs:
        return two_band_ages()
    tot_mem = sum(r["members"] for r in recs if r["members"])
    bands = []
    for r in recs:
        base = ((r["members"] or 0.0) / tot_mem) if tot_mem > 0 else (r["pct"] / 100.0)
        bands.append(AgeBand(r["label"], base, r["visits"], r["age_from"], r["members"]))
    return normalize_age_bands(bands)
...
with st.sidebar:
    ...
    verify_pass = st.checkbox(
        ...
        help="同じ条件でナビィをもう一度検索して漏れを確認します。通常は結果が変わらず"
             "1店あたり1〜2分増えるだけなので既定OFF。取りこぼしが疑われるときだけON。",
    )
    use_osm = st.checkbox(
    ...
    # ── v1.4: 年齢区分 ─────────────────────────────────────────────────
    ff_age_mode = st.radio(
        "年齢区分", ["かんたん（65歳以上／未満）", "詳細（年齢層別・会員データがあるとき）"],
        index=0, key="ff_age_mode",
        help="スーパー様から年齢層別の会員数をいただけたときは「詳細」を選びます。"
             "詳細の既定値は2区分版と同じ水準に較正してあるので、切り替えただけでは予測はほぼ動きません"
             "（全国の年齢構成で差0.01%未満）。",
    )
    ff_detail_ages = ff_age_mode.startswith("詳細")

    if not ff_detail_ages:
        ff_r65 = st.number_input("65歳以上の比率（商圏の高齢化率）", 0.0, 1.0, DEFAULT_R65, 0.01,
                                 help="来店客の年齢構成。会員の年齢データがあればその比率、なければ商圏の高齢化率。")
        c1, c2 = st.columns(2)
        ff_v65 = c1.number_input("65+ の月受診回数", 0.0, 6.0, DEFAULT_V65, 0.1)
        ff_vu65 = c2.number_input("65- の月受診回数", 0.0, 6.0, DEFAULT_VU65, 0.1)
        ff_bands = two_band_ages(ff_r65, ff_v65, ff_vu65)
    else:
        if "ff_age_df" not in st.session_state:
            st.session_state["ff_age_df"] = age_bands_to_df(detail_bands_default())
        st.caption("💡 **実人数**（会員数など）が1行でも入っていれば、構成比はそこから自動計算します"
                   "（実人数が空の行は構成比の列を使用）。行は自由に追加・削除できます。")
        _age_ed = st.data_editor(
            st.session_state["ff_age_df"], num_rows="dynamic", hide_index=True,
            use_container_width=True, key="ff_age_editor",
            column_config={
                "年齢層": st.column_config.TextColumn("年齢層", width="small"),
                "下限年齢": st.column_config.NumberColumn(
                    "下限年齢", min_value=0, max_value=120, step=1,
                    help="その区分の下限。65以上の区分を「65歳以上」として集計・表示します。"),
                "実人数": st.column_config.NumberColumn(
                    "実人数", min_value=0, step=100,
                    help="会員数など。入っていれば構成比より優先されます。"),
                "構成比(%)": st.column_config.NumberColumn(
                    "構成比(%)", min_value=0.0, max_value=100.0, step=0.1, format="%.1f"),
                "月受診回数": st.column_config.NumberColumn(
                    "月受診回数", min_value=0.0, max_value=10.0, step=0.01, format="%.2f",
                    help="その年齢層が1か月に医療機関にかかる平均回数（受診先は問わない）。"),
            },
        )
        st.session_state["ff_age_df"] = _age_ed
        ff_bands = bands_from_df(_age_ed)
        _r65 = sum(b.ratio for b in ff_bands if b.is_senior)
        _vy = sum(b.ratio * b.visits_month for b in ff_bands) * 12.0
        st.caption(f"→ {len(ff_bands)}区分・65歳以上 **{_r65:.1%}**／"
                   f"1人あたり年間受診 **{_vy:.1f}回**"
                   f"（2区分の既定＝{((1-DEFAULT_R65)*DEFAULT_VU65 + DEFAULT_R65*DEFAULT_V65)*12:.1f}回）")
...
def make_fp(uni, bands=None):
    """bands=None なら全候補地共通の年齢区分（サイドバー）を使う。
    候補地ごとに年齢構成をもらえた場合は bands で上書きする（v1.4）。"""
    return FootfallParams(
        enabled=(uni > 0),
        unique_customers_monthly=float(uni),
        age_bands=normalize_age_bands(list(bands) if bands else ff_bands),
...
def compute_candidate(raw):
    ...
    # v1.4: 候補地ごとに年齢構成をもらえた場合はそちらを優先（無ければ全候補地共通）
    age_ovr = st.session_state.get("agemix_multi", {}).get(label)
    fp = make_fp(eff_uni, age_ovr)
...
def _build_footfall_sheet(wb, r):
    """② 集客ベースのシート。v1.4：年齢区分の数に合わせて行数が伸縮する。
    年齢層の「構成比」「月受診回数」を編集すると、獲得枚数まで自動再計算される。"""
    fp = r["fp"]
    bands = fp.bands
    ...
    # ── 年齢層別テーブル（v1.4：区分数ぶん行が伸びる） ──
    AGE_HDR = 7
    for j, htxt in enumerate(["年齢層", "構成比", "月受診回数", "有効客数(人)", "年間受診延べ(回)"], start=1):
        c = ws.cell(row=AGE_HDR, column=j, value=htxt)
        c.font = _HDR
        c.fill = _HDR_FILL
    a0 = AGE_HDR + 1
    for k, b in enumerate(bands):
        rr = a0 + k
        ws.cell(row=rr, column=1, value=b.label)
        # 値は丸めずに書き、表示だけ％にする（丸めるとアプリの計算とわずかにズレるため）
        cr = ws.cell(row=rr, column=2, value=float(b.ratio))
        cr.fill = _INP_FILL
        cr.number_format = "0.0%"
        cv = ws.cell(row=rr, column=3, value=float(b.visits_month))
        cv.fill = _INP_FILL
        cv.number_format = "0.00"
        ws.cell(row=rr, column=4, value=f"=$B$5*B{rr}").fill = _CALC_FILL
        ws.cell(row=rr, column=5, value=f"=D{rr}*C{rr}*12").fill = _CALC_FILL
    a1 = a0 + len(bands) - 1                 # 年齢層テーブルの最終行
    tot = a1 + 1                             # 合計行
    ws.cell(row=tot, column=1, value="合計").font = _BOLD
    for col, f in ((2, f"=SUM(B{a0}:B{a1})"), (4, f"=SUM(D{a0}:D{a1})"), (5, f"=SUM(E{a0}:E{a1})")):
    ...
    ws.cell(row=tot + 1, column=1,
            value="※ 構成比の合計は1.0にしてください（ずれている場合は枚数もその分ずれます）。").font = \
        Font(italic=True, size=9, color="7A8481")

    # ── パラメータ（発行率・院外率・利用率・λ） ──
    PB = tot + 3
    params = [("処方箋発行率", fp.issue_rate), ("院外処方率", fp.external_rate),
              ("当該薬局利用率", fp.use_rate), ("面競合の距離減衰λ(m)", fp.competitor_decay_m)]
    for k, (lab, val) in enumerate(params):
        ws.cell(row=PB + k, column=1, value=lab)
        ws.cell(row=PB + k, column=2, value=val).fill = _INP_FILL
    R_ISSUE, R_EXT, R_USE, R_LAM = PB, PB + 1, PB + 2, PB + 3

    # ── 計算 ──
    CB = PB + 6
    last_comp_hdr = CB + 7                                  # 競合表のヘッダ行
    cs = last_comp_hdr + 1                                  # 競合の先頭行
    ce = max(cs, cs + len(r["classified"]) - 1)             # 競合の最終行
    computed = [
        ("年間受診延べ(回)", f"=E{tot}"),
        ("院外処方プール(枚)", f"=B{CB}*B{R_ISSUE}*B{R_EXT}"),
        ("面競合の実効パワー", f"=SUM(E{cs}:E{ce})"),
        ("シェア", f"=B{R_USE}/(1+B{CB + 2})"),
        ("獲得（年間・枚）", f"=B{CB + 1}*B{CB + 3}"),
        ("獲得（月間・枚）", f"=B{CB + 4}/12"),
    ]
    ...
    for j, htxt in enumerate(["競合薬局名", "候補地から(m)", "実績(枚)", "面=1/門前=0", "重み(自動)"], start=1):
        c = ws.cell(row=last_comp_hdr, column=j, value=htxt)
        c.font = _HDR
        c.fill = _HDR_FILL
    for k, cl in enumerate(r["classified"]):
        ...
        # λ が0のときは距離減衰なし（＝重み1.0）。Pythonの footfall_competitor_power と一致させ #DIV/0! を防ぐ
        ws.cell(row=rr, column=5,
                value=f"=IF(D{rr}=1,IF(C{rr}>0,C{rr}/12000,1)"
                      f"*IF($B${R_LAM}>0,EXP(-B{rr}/$B${R_LAM}),1),0)").fill = _CALC_FILL
    for col, w in zip("ABCDE", [30, 14, 14, 14, 18]):
...
def _build_raw_ph_sheet(wb, results):
    ...
    for col, w in zip("ABCDEFGHIJKLMN",
                      [8, 30, 9, 11, 11, 14, 24, 9, 17, 12, 14, 34, 14, 46]):
        ws.column_dimensions[col].width = w
    ws.auto_filter.ref = f"A1:N{max(r_i - 1, 1)}"
    return ws
...
def _build_client_sheet(ws, results):
    ...
    for k, txt in enumerate([
        ...
        "※ 周辺の医療機関・薬局は、厚生労働省「医療情報ネット（ナビィ）」およびOpenStreetMapの公開情報に基づきます。",
...
if col_clear.button("🗑 結果をクリア", use_container_width=True):
    for k in ("mk_multi", "exp_multi", "med_edit", "ph_edit", "agemix_multi"):
        st.session_state[k] = {}
...
if raws:
    ...
    if c["foot"]:
        ...
        if abs(new_exp - c["exposure"]) > 1e-9:
            ...
            st.rerun()
        fo, fp = c["foot"], c["fp"]
        _agesrc = "候補地別（この候補地だけの年齢構成）" if c.get("age_override") else "全候補地共通（サイドバー）"
        st.markdown(
            f"- 館の来店客数 {c['uni']:,.0f}人 × **周知率 {c['exposure']:.2f}** = 有効客数 **{c['eff_uni']:,.0f}人**"
            f"（65+ {fo['u65']:,.0f} / 65− {fo['u_under']:,.0f}）\n"
            f"- 年齢区分 **{fo['n_bands']}区分**（{_agesrc}）\n"
        ...
        # ── v1.4: 年齢層別の内訳（どの年齢層が原資を生んでいるかを見る） ──
        st.markdown("###### 年齢層別の内訳")
        _bt = pd.DataFrame([{
            "年齢層": b["label"],
            "構成比": b["ratio"],
            "有効客数(人)": round(b["customers"]),
            "月受診回数": b["visits_month"],
            "年間受診延べ(回)": round(b["annual_visits"]),
            "院外処方プール(枚)": round(b["annual_visits"] * fp.issue_rate * fp.external_rate),
            "獲得(枚/年)": round(b["annual_visits"] * fp.issue_rate * fp.external_rate * fo["share"]),
            "構成": (b["annual_visits"] / fo["annual_visits"]) if fo["annual_visits"] > 0 else 0.0,
        } for b in fo["bands"]])
        st.dataframe(_bt, hide_index=True, use_container_width=True, column_config={
            "構成比": st.column_config.NumberColumn("構成比", format="%.1f%%", help="来店客の年齢構成"),
            "月受診回数": st.column_config.NumberColumn("月受診回数", format="%.2f"),
            "構成": st.column_config.ProgressColumn("枚数への寄与", format="%.1f%%",
                                                 min_value=0.0, max_value=1.0),
        })
        st.caption("※「構成比」「構成（枚数への寄与）」は割合表示です。各年齢層の獲得枚数の合計＝②の獲得枚数。")

        # ── v1.4: この候補地だけ年齢構成を差し替える（店舗別の会員データ用） ──
        with st.expander(f"👥 この候補地（{sel}）だけ年齢構成を差し替える", expanded=False):
            st.caption("店舗ごとに年齢層別の会員数をいただけた場合はここに入力してください。"
                       "**実人数**を入れれば構成比は自動計算されます。"
                       "客数（枚数の絶対量）は上の『月間ユニーク客数 × 周知率』のままで、"
                       "ここで変わるのは**年齢の内訳だけ**です。")
            _ovr_all = st.session_state.setdefault("agemix_multi", {})
            _cur = _ovr_all.get(sel) or [AgeBand(b["label"], b["ratio"], b["visits_month"],
                                                 b["age_from"], b["members"]) for b in fo["bands"]]
            _ed = st.data_editor(
                age_bands_to_df(_cur), num_rows="dynamic", hide_index=True,
                use_container_width=True, key=f"agemix_ed_{sel}",
                column_config={
                    "年齢層": st.column_config.TextColumn("年齢層", width="small"),
                    "下限年齢": st.column_config.NumberColumn("下限年齢", min_value=0, max_value=120, step=1),
                    "実人数": st.column_config.NumberColumn("実人数", min_value=0, step=100),
                    "構成比(%)": st.column_config.NumberColumn("構成比(%)", min_value=0.0, max_value=100.0,
                                                            step=0.1, format="%.1f"),
                    "月受診回数": st.column_config.NumberColumn("月受診回数", min_value=0.0, max_value=10.0,
                                                           step=0.01, format="%.2f"),
                },
            )
            _b1, _b2 = st.columns(2)
            if _b1.button("🔄 この候補地に反映して再計算", key=f"agemix_apply_{sel}", type="primary"):
                _ovr_all[sel] = bands_from_df(_ed)
                st.session_state.pop(f"agemix_ed_{sel}", None)
                st.rerun(scope="app")
            if _b2.button("↩ 共通設定に戻す", key=f"agemix_reset_{sel}",
                          disabled=not c.get("age_override")):
                _ovr_all.pop(sel, None)
                st.session_state.pop(f"agemix_ed_{sel}", None)
                st.rerun(scope="app")
