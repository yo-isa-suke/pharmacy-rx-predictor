...
def calc_commercial_radius(
    ...
    if pharmacy_type == PHARMACY_TYPE_SUPERMARKET:
        ...
        r = SUPERMARKET_RADIUS_TABLE.get(band, 1_200)
        return r, (
            f"スーパー・ドラッグストア内薬局（密度帯: {band} / {density:,}人/km²）"
            f" → 食品スーパー一次商圏 {r}m"
            f"（国土交通省・経産省商業統計ベース）"
        )
...
class Method2Predictor:
    def predict(
        ...
        methodology = [
            "### 方法②（商圏人口動態アプローチ）ロジック",
            "",
            "**算出式（v4.2）**: 商圏人口 × 密度帯別年齢分布 × 年齢層別受診率 × 処方箋発行率(69%)",
            "× 院外処方率(79.0%) × **処方箋流入係数** × 当薬局市場シェア",
            f"**v4.2改善**: 密度帯`{density_band_label}`の年齢分布を適用（65歳以上: {elderly_ratio:.1%}）",
...
def main() -> None:
    st.set_page_config(
        page_title="薬局 処方箋枚数 多面的予測 v4.3",
        page_icon="💊",
        layout="wide",
        initial_sidebar_state="collapsed",
    )
    st.title("💊 薬局 年間処方箋枚数 多面的予測ツール v4.3")
