"""
医療機関検索ツール v2.0 — 厚生労働省ナビイ + OpenStreetMap 統合版
...
v2.0 主な改善点:
  - OSM Overpass API による半径圏内の高速施設取得（ジオコーディング不要）
  - kikanKbn 自動判定（診療所=2 / 病院=1 / 歯科=3 など kikanCd 先頭桁から推定）
  - MHLW 詳細ページの全タブコンテンツを正しくパース
  - 院内処方フィールド名を正確に指定（「院内処方の有無」「院外処方の有無」）
  - 診療時間テーブルから週診療日数を算出
  - 医療実績タブから外来患者数を抽出
  - 詳細取得上限デフォルト 30件（最大 80件）に引き上げ
  - デバッグモード: 全取得フィールドを表示
"""
...
class GeocoderService:
    ...
    HEADERS = {"User-Agent": "MedicalFinderTool/2.0"}
    ...
    def _clean(self, address: str) -> str:
        ...
        return re.sub(r"\s+", " ", a.translate(trans).replace("　", " ")).strip()
    ...
    def geocode(self, address: str) -> Optional[Tuple[float, float]]:
        ...
        if result:
            return result
        short = re.sub(r"\d+(?:階|F|棟|号室|号).*$", "", clean).strip()
        if short and short != clean:
            result = self._gsi(short)
            if result:
                return result
        time.sleep(1.0)
        return self._nominatim(clean)
