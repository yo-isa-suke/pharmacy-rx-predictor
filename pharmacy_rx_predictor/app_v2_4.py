"""
薬局 年間処方箋枚数 多面的予測ツール v2.4
==========================================
v2.3 からの改善点:
  1. 門前クリニック未検出問題の対応（主要修正）
     - 課題: OSM（OpenStreetMap）は日本の中小クリニックの収録が不完全で、
       実際には門前に存在するクリニックが検索結果に現れないことがある。
       → 方法①（近隣医療機関アプローチ）が大幅に過小推計になる。
     - 対応A: 「近隣施設を手動追加」機能
         分析結果画面で OSM 未収録のクリニックを手動で追加できる。
         名称・診療科・外来患者数/日・距離(m)・院内薬局の有無を入力し
         「再計算」ボタンで MHLW/OSM 再検索なしに予測を更新。
     - 対応B: MHLW医療機関DBからクリニックを検索して追加
         クリニック名（一部）で MHLW 医療機関データベースを検索し、
         候補を選んで追加できる（外来患者数はMHLWから自動取得を試みる）。
     - 対応C: 乖離警告と未検出施設の逆算表示
         MHLW実績値と方法①予測の乖離が大きい場合に警告を表示し、
         乖離から「外来 約X人/日のクリニックが未検出の可能性あり」と逆算。
  2. OSM検索タグの拡充
     - amenity=医院, amenity=診療所（日本語タグ）にも対応
     - healthcare=clinic/doctor に加えて healthcare=physiotherapist 等を追加
     - name:ja タグを優先した名前取得（既存）を補強

v2.3 の内容（前バージョン）:
  - 方法②市場シェア計算バグ修正（1/d² → 実効競合数モデル）
v2.2 の内容（前々バージョン）:
  - 国土地理院ジオコーダー採用 / データソース表示 / 新規開局予測モード追加
"""
...
def build_competitor_map(
    ...
    for fac in medical_facilities:
        ...
        manual_note = "<br><b style='color:#e67e00'>※ 手動追加施設（OSM未収録）</b>" if fac.is_manual else ""
        popup_html = (
            f"<b>{label} {fac.name}</b>{inhouse}{manual_note}<br>"
...
def calc_implied_missing_facility(actual_rx: int, predicted_rx: int) -> Optional[Dict]:
    ...
    逆算は一般内科の処方箋発行率(0.76)×院外処方率(0.745)で行う。
    """
    ...
    or_rate = NATIONAL_STATS["outpatient_rx_rate"]  # 0.745
...
def recalculate_with_manual_facilities(analysis: "FullAnalysis") -> None:
    """
    手動追加施設を含めて方法①②を再計算し、session_state を更新する。
    ...
    merged_medical = list(analysis.nearby_medical) + manual_facs

    m1 = Method1Predictor().predict(
        analysis.pharmacy_lat, analysis.pharmacy_lon,
        merged_medical, analysis.nearby_pharmacies,
        mode_label="方法①: 近隣医療機関（OSM＋手動追加）",
    ...
    # dataclasses.replace() で更新フィールドのみ差し替え
...
def render_gap_and_manual_input(analysis: "FullAnalysis") -> None:
    """
    v2.4: 乖離警告 + 近隣施設手動追加 + 再計算セクション

    MHLW実績と方法①予測の乖離が大きい場合に警告を出し、
    OSMで未収録のクリニックを手動で追加して再計算できる。
    """
    actual = analysis.mhlw_annual_rx
    m1 = analysis.method1
    ...
    if implied:
        st.warning(
            ...
            f"以下の「近隣施設を手動追加」セクションで補完し、再計算してください。"
        )
    ...
    with st.expander(
        ...
        if clinic_cands:
            ...
            mhlw_sp      = col_b.selectbox("診療科", list(SPECIALTY_RX_RATES.keys()),
                                           key="mhlw_sp_v4")
            mhlw_op      = col_c.number_input("外来患者数/日", 1, 500, 50, key="mhlw_op_v4",
                                              help="MHLW取得ボタンで自動取得を試みることもできます")
...
def main() -> None:
    st.set_page_config(
        page_title="薬局 処方箋枚数 多面的予測 v2.4",
        page_icon="🔬",
        layout="wide",
        initial_sidebar_state="collapsed",
    )
    st.title("🔬 薬局 年間処方箋枚数 多面的予測ツール v2.4")
    st.markdown(
        "🆕 **v2.4の追加機能**: OSMで未検出のクリニックを**手動追加して再計算**できる機能を追加。"
        "MHLWへの実績乖離が大きい場合の警告・逆算表示も対応。"
    )
        ...
        with tab_map:
            if analysis.pharmacy_lat and analysis.pharmacy_lon:
                ...
                m = build_competitor_map(
                    ...
                    analysis.pharmacy_lat, analysis.pharmacy_lon,
                    analysis.nearby_medical + _manual_for_map,
                    analysis.nearby_pharmacies,
