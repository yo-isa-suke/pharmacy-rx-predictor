...
class Method2Predictor:
    def predict(
        ...
        methodology = [
            "### 方法②（商圏人口動態アプローチ）ロジック",
            "",
            "**算出式（v3.2）**: 商圏人口 × 年齢層別受診率 × 処方箋発行率(69%)",
    ...
    def _inflow_coefficient(density: int) -> Tuple[float, str]:
        """
        処方箋流入係数（v3.2新規）: 商圏居住人口から算出した処方箋総数に対する補正係数。
        ...
        ■ 校正根拠
          ...
          競合3件（実効N=4.0、シェア20%）での校正例:
            高密度都市(16,000/km²): 30,470 × 1.60 × 20% ≒  9,750枚/年  ← 都市中央値圏
            都市郊外(8,000/km²):   27,081 × 1.40 × 20% ≒  7,582枚/年  ≈ 全国中央値
            中密度郊外(4,000/km²): 21,165 × 1.25 × 20% ≒  5,291枚/年  ← 競合3件では低め
            ※ 競合2件(N=2.5, シェア28.6%)なら: 21,165×1.25×28.6%≒7,565枚/年 ≈ 中央値
        """
        if density >= 10_000:
            return 1.60, "大都市中心部（昼間人口・大病院集積による流入が大）"
        elif density >= 5_000:
            return 1.40, "都市郊外（住宅・商業・医療の混在による流入）"
        elif density >= 2_000:
            return 1.25, "中密度住宅地（近隣からの受療圏流入）"
        elif density >= 500:
            return 1.12, "地方都市（受療圏が広く流入は中程度）"
        else:
            return 1.05, "農村部（受療圏は基本的に地域内完結）"
