...
class Method1Predictor:
    ...
    def predict(
        ...
        methodology = [
            ...
            "**v3.2シェアロジック**: 医療機関50m以内に競合薬局がある場合は「既存門前薬局あり」",
            "と判定しベースシェアを大幅割引（≤50m:15%/≤150m:10%/≤300m:8%/300m超:5%）。",
            "競合なし時は従来通り（≤50m:75%/≤150m:50%/≤300m:30%/300m超:15%）。",
            "",
    ...
    def _calc_share(self, fac, ph_lat, ph_lon, competitors) -> Tuple[float, str]:
        """
        医療機関ごとの当薬局集客シェアを推計する（v3.2 大幅改善）

        ■ 改善ポイント（v3.2）:
          医療機関の50m以内に競合薬局が既に存在する場合（＝既存門前薬局あり）、
          その医療機関の処方箋はほぼ門前薬局に流れるため、当薬局のシェアを大幅割引。
          固定 has_gate_pharmacy ×0.4 フラグを廃止し、競合薬局の実際の立地で動的判定。

        ■ シェアテーブル（当薬局からの距離 × 既存門前薬局の有無）:
          【門前薬局なし（従来通り）】   ≤50m: 75% / ≤150m: 50% / ≤300m: 30% / >300m: 15%
          【門前薬局あり（大幅割引）】   ≤50m: 15% / ≤150m: 10% / ≤300m:  8% / >300m:  5%

        その後、医療機関300m圏内の競合薬局数で Huff 型按分を実施。
        """
        dist = OverpassSearcher._haversine(fac.lat, fac.lon, ph_lat, ph_lon)

        # ── 既存門前薬局チェック: 医療機関から50m以内に競合薬局があるか ──────
        gate_comps = [
            p for p in competitors
            if OverpassSearcher._haversine(fac.lat, fac.lon, p.lat, p.lon) <= 50
        ]
        has_existing_gate = len(gate_comps) > 0

        if has_existing_gate:
            # 既存門前薬局あり → ベースシェアを大幅に割引
            if dist <= 50:
                base, reason = 0.15, f"50m以内だが既存門前薬局あり（{gate_comps[0].name[:12]}等）→最大15%"
            elif dist <= 150:
                base, reason = 0.10, f"150m以内・既存門前薬局あり（{gate_comps[0].name[:12]}等）→最大10%"
            elif dist <= 300:
                base, reason = 0.08, f"300m以内・既存門前薬局あり（{gate_comps[0].name[:12]}等）→最大8%"
            else:
                base, reason = 0.05, f"300m超・既存門前薬局あり（{gate_comps[0].name[:12]}等）→最大5%"
        else:
            # 既存門前薬局なし → 従来通りのベースシェア
            if dist <= 50:
                base, reason = 0.75, "50m以内（実質門前・競合なし）"
            elif dist <= 150:
...
class Method2Predictor:
    ...
    def _market_share(
        ...
        v3.2 追加: 「門前的競合」（医療機関から100m以内に位置する競合薬局）は
            ...
            シェア = 1 / (実効競合数 + 1)
            上限: 80% （競合ゼロ門前。残20%は商圏外流出・他エリア受診として控除）
...
def main() -> None:
    st.set_page_config(
        page_title="薬局 処方箋枚数 多面的予測 v3.2",
        page_icon="🔬",
        layout="wide",
        initial_sidebar_state="collapsed",
    )
    st.title("🔬 薬局 年間処方箋枚数 多面的予測ツール v3.2")
    st.markdown(
        "📊 **v3.2 データ精度・シェアロジック強化**: "
        "競合薬局の処方箋枚数を**自動取得**（常時実行）、"
        "OSM薬局検索タグを拡充（**shop=pharmacy** 等も対応）、"
        "既存門前薬局がある場合のシェアを**動的割引**に修正（方法①・方法②）。"
    )
